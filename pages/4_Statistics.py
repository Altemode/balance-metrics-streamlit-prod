...
############## ############## PAGE 4 STATISTICS ############# ############# ############## ########################
st.set_page_config(
    page_title="School of Physical Education and Sports Science balance Statistics",
    page_icon="✅",
    layout="wide",
...
# top-level filters
jump_trial_filter = st.selectbox("Select the age of the person!", pd.unique(df["age"]))

# creating a single-element container
placeholder = st.empty()

# dataframe filter
df_balance_table = df[df["age"] == jump_trial_filter]

# near real-time / live feed simulation
#for seconds in range(100):

#df_balance_table["age_new"] = df_balance_table["age"] * np.random.choice(range(1, 5))
#df_balance_table["balance_new"] = df_balance_table["balance"] * np.random.choice(range(1, 5))

# creating KPIs
total_trials = df_balance_table['age'].count()

avg_age = np.mean(df_balance_table["age"])

avg_height = np.mean(df_balance_table["height"])

avg_weight = np.mean(df_balance_table["weight"])


# count_married = int(
#     df_balance_table[(df_balance_table["marital"] == "married")]["marital"].count()
#     + np.random.choice(range(1, 30))
# )

# balance = np.mean(df_balance_table["balance_new"])
...
with placeholder.container():
    ...
    with fig_col1:
        ...
        fig = px.density_heatmap(
            data_frame=df_balance_table, y="age", x="height"
        )
        fig.update_layout(
    ...
    with fig_col2:
        ...
        fig2 = px.density_heatmap(
            data_frame=df_balance_table, y="age", x="weight"
        )
        fig2.update_layout(
             margin=dict(l=0, r=0, t=10, b=60),
             #paper_bgcolor="LightSteelBlue",
    ...
    with fig_col3:
        ...
        fig3 = px.histogram(data_frame=df_balance_table, x="age")
        fig3.update_layout(
    ...
    with col2:
        ...
        st.dataframe(df[['id', 'fullname','occupy','age','height','weight','email','created_at']])
